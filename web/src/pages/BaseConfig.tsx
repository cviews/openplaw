...
import type { OpenplawFileConfig } from '../lib/types';
...
export function BaseConfig() {
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      try {
        setLoading(true);
        const data = await configApi.getOpenplaw();
        if (!cancelled) {
  ...
  const doSave = async () => {
    let parsed: OpenplawFileConfig;
    try {
      parsed = JSON.parse(configText) as OpenplawFileConfig;
    } catch (err) {
    ...
    try {
      setSaving(true);
      await configApi.putOpenplaw(parsed);
    ...
    <div>
      ...
      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)} title="Confirm Save">
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
          ...
          <Button variant="primary" onClick={doSave} disabled={saving}>Confirm</Button>
        </div>
      </Modal>
