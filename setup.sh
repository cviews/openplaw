...
config = {'bots': bots, 'groups': groups, 'agents': existing.get('agents', {'directory': ['~/.config/openplaw/agents']}), 'mcp': existing.get('mcp', {'autoRegister': True}), 'ports': existing.get('ports', {'gateway': 3000, 'gatewayHost': '0.0.0.0', 'health': 9090, 'opencode': 4096, 'hub': 4097, 'web': 4098})}
